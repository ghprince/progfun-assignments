...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet = {
    val leftMost = left.mostRetweetedAcc(acc)
    val rightMost = right.mostRetweetedAcc(acc)
    val childMost = if (leftMost.retweets > rightMost.retweets) leftMost else rightMost
    if (childMost.retweets > elem.retweets)
      if (childMost.retweets > acc.retweets) childMost else acc
    else
